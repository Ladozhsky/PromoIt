...
export class ListCreation implements IListCreation {
  ...
  public async createListOfRetweets(): Promise<retweet[]> {
    const userIds: twitterUserIds[] = await dbGetService.getAllCollumnData(
      Queries.TwitterUserIds
    );
    const campaignsData: campaignIdHashtag[] =
      await dbGetService.getAllCollumnData(Queries.CampainHashtag);
    ...
    try {
      for (let i = 0; i < userIds.length; i++) {
        for (let j = 0; j < campaignsData.length; j++) {
          const tweets = await twitterService.searchTweetsByIdHashtag(
            userIds[i].email_twitter_id,
            campaignsData[j]
          );
  ...
  public async createListOfTransaction(): Promise<transaction[]> {
    const lastRetweetAr: localRetweet[] = await dbGetService.getAllCollumnData(
      Queries.LastNotProseedRetweet
    );
    const top2RetweetAr: localRetweet[] = await dbGetService.getAllCollumnData(
      Queries.MostRetweetedProsessedTweet
    );
    const transactionArray: transaction[] = [];
    try {
      for (let i = 0; i < lastRetweetAr.length; i++) {
        let mostRetweetedTweet: localRetweet =
          top2RetweetAr[
            top2RetweetAr.findIndex(
              (item) =>
                item.twitter_user_id === lastRetweetAr[i].twitter_user_id &&
                item.campaign_id === lastRetweetAr[i].campaign_id
            )
          ];
        if (mostRetweetedTweet === undefined) {
          transactionArray.push(
            this.parselocalTransaction(
              lastRetweetAr[i],
              lastRetweetAr[i].retweets + 1,
              "FTP"
            )
          ); // FTP - first tweet posting
        }
        // if have new retweets
          ...
          transactionArray.push(
            this.parselocalTransaction(
              lastRetweetAr[i],
              lastRetweetAr[i].retweets - mostRetweetedTweet.retweets,
              "RC"
            )
          ); // RC - retweet counting
        } else {
